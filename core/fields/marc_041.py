...
원본: 041/041.py 의 LangFieldBuilder 클래스 전체(1976줄, docs/INTEGRATION_SURVEY.md
041 섹션 참고). 저자/역자 소개글(Bio) 크롤링(AladinAuthorScraper)은 원칙 #9에 따라
api/aladin_scraper.py로 분리했고, 이 모듈은 그것을 가져다 쓴다.

이식 시 적용한 원칙 (docs/INTEGRATION_PRINCIPLES.md):
  - #3  OpenAI 클라이언트는 함수 인자로 주입 (원본이 이미 이 방식 — 그대로 유지).
        모델명은 core.config.Settings.openai_model_041을 app.py가 넘겨준다.
  - #6  원본은 get_kormarc_tags()에서 예외 발생 시 "📕 예외 발생: …" 문자열을
        tag_041 자리(성공 값 자리)에 섞어 반환하는 안티패턴이 있었다 — 여기서는
        예외 시 (None, None, original_title)을 반환하도록 고쳤다(디버그 로그에는
        그대로 남긴다).
  - #7  디버그 로깅은 core.debug_log.dbg/dbg_err를 기본값으로 쓰고 "[041]" 프리픽스를
        붙인다. 원본처럼 생성자에서 dbg_fn/dbg_err_fn을 주입받는 것도 계속 허용한다.
  - #8  Optional[X] 등 typing 모듈 스타일 타입힌트 유지.

수정한 버그:
  - 원본 1780행 부근 item.get("categoryText", "") 단독 호출 — 실제 알라딘 API
    필드명은 categoryName이므로 item.get("categoryName") or item.get("categoryText", "")
    로 수정(914행 부근의 _build_book_info는 원래도 올바르게 처리하고 있었음).

041 담당자 최종본(marc_041.py)에서 가져온 "로직" 강화 사항 — 단, 아래 항목들과 달리
AladinAuthorScraper를 이 파일에 다시 인라인하지는 않았다. Bio 크롤링은 여전히
api/aladin_scraper.py의 AladinAuthorScraper를 그대로 가져다 쓰는 구조를 유지한다:
  - 엮은이(편자) 판별(_role_is_editor / _has_editor_in_item) 추가. 저자 없이 엮은이만
    있는 경우, 원제 유무로 "국내 편집물 vs 외국 원작 편집물"을 가르는 가드(editor bypass)를
    determine_h_language / get_kormarc_tags 여러 지점에 추가.
  - 순수 한자(가나 없음) 원제의 chi/jpn 오판 방지 — detect_language_by_unicode가 이 경우
    chi를 즉시 확정하지 않고 und를 반환하도록 수정. determine_h_language에서 순수 한자
    원제를 감지하면 GPT Payload에 chi/jpn 판별 힌트를 별도 주입하고, 시스템 프롬프트에도
    "규칙 F-2"와 엮은이 특수 규칙을 추가.
  - Bio 크롤링·Payload 판정이 모두 실패했을 때 raw author 문자열을 그대로 GPT에 전달하는
    단순 폴백 gpt_guess_original_lang_simple() 추가(1215_main.py 방식 이식).
  - 역자·원제·엮은이가 모두 없으면 Bio 크롤링·GPT 호출 자체를 스킵하고 즉시 und를 반환하는
    조기 종료 최적화를 determine_h_language / get_kormarc_tags 양쪽에 추가.
  - _build_book_info에 raw author 원시 문자열을 GPT 힌트로 추가.
  - GPT 호출(chat.completions.create) 직후 core.token_tracker.add()를 호출해 다른 필드
    모듈(245/300/653)과 동일하게 토큰 사용량 집계에 반영되도록 함(원본에는 없던 계측 코드).
  - 맨 아래에 app.py가 호출하는 진입점 build_041_546()을 추가.
    (LangFieldBuilder 자체의 클래스/메서드 시그니처는 변경하지 않음.)

【사용 예시】
    from core.fields.marc_041 import build_041_546, LangFieldBuilder

    tag_041, tag_546, orig_title = build_041_546(item, {}, openai_client=client, model=model)
    mrk_041 = LangFieldBuilder.as_mrk_041(tag_041)
    mrk_546 = LangFieldBuilder.as_mrk_546(tag_546)
"""
...
# ═══════════════════════════════════════════════════════════════
# 2. 역할 판별 헬퍼 (저자/역자/엮은이 판별 — 041 자체 판정용)
# ═══════════════════════════════════════════════════════════════
# 참고: api.aladin_scraper 모듈 안에도 동명 함수가 있으나, 그건 Bio 수집(스크래핑)에 쓰이는
# 별개의 정의다. 여기 정의는 041의 $h 판정 로직(엮은이 우회, 저자/역자 이름 파싱 등) 전용이며
# 서로 독립적으로 유지한다(원본 041.py도 동일 구조).

def _role_is_translator(role: str) -> bool:
    """역자 역할 여부 판별 (api.aladin_scraper의 동명 함수와 별개 — 041 자체 판별 기준)."""
    r = (role or "").strip()
    if not r:
        return False
...
class LangFieldBuilder:
    ...
    dbg_fn
        디버그 메시지 출력 함수.  기본값 core.debug_log.dbg("[041]", ...).
    dbg_err_fn
        에러 메시지 출력 함수.  기본값 core.debug_log.dbg_err("[041]", ...).
    """
    ...
    def __init__(
        ...
        self._model   = model
        self._dbg     = dbg_fn     or (lambda *a: dbg("[041]", *a))
        self._dbg_err = dbg_err_fn or (lambda *a: dbg_err("[041]", *a))
...
""".strip()
    ...
    def get_kormarc_tags(
        ...
        (tag_041, tag_546, original_title)
          ...
          - 예외 발생 시        : (None, None, original_title)  (디버그 로그에는 예외 내용 기록)
        """
        item   = item   or {}
        detail = detail or {}
        ...
        # 원본 041.py 버그 수정: 실제 알라딘 API 필드명은 categoryText가 아니라
        # categoryName이다 (docs/INTEGRATION_SURVEY.md 041 섹션 참고).
        category_text = (
            item.get("categoryName")
            or item.get("categoryText", "")
        ...
        try:
            ...
            self._dbg_err(f"[041] get_kormarc_tags 예외: {e}")
            return None, None, original_title
...
def build_041_546(
    ...
    *,
    ...
    """
    041/546 MRK 문자열과 원제를 반환한다.

    Args:
        item: 알라딘 API item dict (api/aladin_client.OPT_RESULT_FULL로 조회된 것).
        detail: 크롤링으로 보강된 dict — 현재 어느 필드 모듈도 이를 생성하지 않으므로
                {}로 호출해도 안전(원서 언어 판정은 item만으로 충분히 동작).
        openai_client: 041 판정용 GPT 호출에 쓸 클라이언트 (없으면 규칙 기반만 사용).
        model: openai_client 사용 시 모델명 (core.config.Settings.openai_model_041).

    Returns:
        (tag_041, tag_546, original_title)
        번역서가 아니면 (None, None, original_title).
    """
    builder = LangFieldBuilder(openai_client=openai_client, model=model)
    return builder.get_kormarc_tags(item, detail or {})
